...
class Robot:
    ...
    def forward_kinematics(self, thetas):
        ...
        #Because frame 1 and n (end effector) are unique, fill those seperately from the loop. Frame 1 only has rotation
        frames[:, :, 0] = np.array([[np.cos(thetas[0, 0]), -np.sin(thetas[0, 0]), 0],
                                    [np.sin(thetas[0, 0]), np.cos(thetas[0, 0]), 0],
                                    [0, 0, 1]])
        for i in range(1, n):
            link = self.link_lengths[i-1, 0] #might need to fix this if off by one
            theta = thetas[i, 0]
            frames[:, :, i] = frames[:, :, i-1] @ np.array([[np.cos(theta), -np.sin(theta), link],
                                                            [np.sin(theta), np.cos(theta), 0],
                                                            [0, 0, 1]])
        #Now handle frame n+1 which only has translation
        last_link = self.link_lengths[-1, 0]
        frames[:, :, n] = frames[:, :, n-1] @ np.array([[1, 0, last_link],
                                                        [0, 1, 0],
                                                        [0, 0, 1]])
