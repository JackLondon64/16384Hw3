...
def forward_kinematics_RR(theta1, theta2):
    ...
    H_1_0 = np.array([[np.cos(theta1), -np.sin(theta1), 0],
                      [np.sin(theta1), np.cos(theta1), 0],
                      [0, 0, 1]])
    H_2_0 = H_1_0 @ np.array([[np.cos(theta2), -np.sin(theta2), l1],
                              [np.sin(theta2), np.cos(theta2), 0],
                              [0, 0, 1]])
    H_3_0 = H_2_0 @ np.array([[1, 0, l2],
                              [0, 1, 0],
                              [0, 0, 1]])
    # ============ END STUDENT SECTION ==============
...
def jacobian_RR(theta1, theta2):
    ...
    #x = l1c1 + l2c12 and y = l1s1 + l2s12 so take the derivative of these wrt theta1 and theta2 for j as follows:
